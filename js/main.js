...
(function () {
  ...
  if (hlTrack && hlPrev && hlNext) {
    const hlCards  = Array.from(hlTrack.querySelectorAll('.pse-highlights__card'));
    let hlIndex    = 0;
    let hlAuto;
    ...
    function updateHighlights() {
      ...
      hlTrack.style.transition = reducedMotion ? 'none' : 'transform 0.45s cubic-bezier(.4,0,.2,1)';
    ...
    hlPrev.addEventListener('click', () => { hlIndex = Math.max(0, hlIndex - 1); updateHighlights(); resetAuto(); });
    hlNext.addEventListener('click', () => { const max = Math.max(0, hlCards.length - getVisibleCount()); hlIndex = Math.min(hlIndex + 1, max); updateHighlights(); resetAuto(); });

    // Keyboard navigation
    hlTrack.addEventListener('keydown', (e) => {
      if (e.key === 'ArrowLeft') { hlPrev.click(); }
      if (e.key === 'ArrowRight') { hlNext.click(); }
    });

    // Touch swipe
    let hlTouchX = null;
    hlTrack.addEventListener('touchstart', (e) => { hlTouchX = e.touches[0].clientX; }, { passive: true });
    hlTrack.addEventListener('touchend', (e) => {
      if (hlTouchX === null) return;
      const diff = hlTouchX - e.changedTouches[0].clientX;
      if (Math.abs(diff) > 40) { diff > 0 ? hlNext.click() : hlPrev.click(); }
      hlTouchX = null;
    }, { passive: true });
    ...
    function startAuto() {
      ...
      hlAuto = setInterval(() => {
        ...
        if (hlIndex >= max) hlIndex = 0; else hlIndex++;
        updateHighlights();
      }, 4000);
    }
    function resetAuto() { clearInterval(hlAuto); startAuto(); }
  ...
  if (chooseTrack && chooseDots && choosePrev && chooseNext) {
    ...
    function updateChoose() {
      if (!isMobile()) { showChooseMobileControls(false); return; }
    ...
    choosePrev.addEventListener('click', () => { idx = Math.max(0, idx - 1); updateChoose(); });
    chooseNext.addEventListener('click', () => { idx = Math.min(cards.length - 1, idx + 1); updateChoose(); });
    dots.forEach((d, i) => d.addEventListener('click', () => { idx = i; updateChoose(); }));

    // Touch swipe on track
    let cTouchX = null;
    chooseTrack.addEventListener('touchstart', (e) => { cTouchX = e.touches[0].clientX; }, { passive: true });
    chooseTrack.addEventListener('touchend', (e) => {
      if (cTouchX === null) return;
      const diff = cTouchX - e.changedTouches[0].clientX;
      if (Math.abs(diff) > 40) { diff > 0 ? chooseNext.click() : choosePrev.click(); }
      cTouchX = null;
    }, { passive: true });

    // Sync dots on native scroll
    chooseTrack.addEventListener('scroll', debounce(() => {
      if (!isMobile()) return;
  ...
  /* ─────────────────────────────────────────────────────
     MARQUEE – pause on focus inside
  ───────────────────────────────────────────────────── */
  document.querySelectorAll('.pse-schools__track-wrap').forEach(wrap => {
    wrap.addEventListener('focusin', () => {
      wrap.querySelectorAll('.pse-schools__track').forEach(t => t.style.animationPlayState = 'paused');
    });
    wrap.addEventListener('focusout', debounce(() => {
      wrap.querySelectorAll('.pse-schools__track').forEach(t => t.style.animationPlayState = '');
    }, 100));
  });
